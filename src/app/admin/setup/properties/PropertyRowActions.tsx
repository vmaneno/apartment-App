...
export function PropertyRowActions({ property }: { property: Property }) {
  ...
  async function handleDelete() {
    setLoading(true)
    const res = await fetch(`/api/setup/properties/${property.id}`, { method: 'DELETE' })
    const json = await res.json().catch(() => ({}))
  ...
  const field = (label: string, name: string, defaultValue: string, req = false) => (
    <div>
      <label className="block text-xs font-medium mb-1" style={{ color: 'var(--text-muted)' }}>{label}</label>
      <input name={name} defaultValue={defaultValue} required={req} className={cls} style={sx} />
    </div>
  )
      ...
      {editing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.6)' }}>
          <div className="rounded-xl p-6 w-full max-w-lg shadow-xl overflow-y-auto max-h-[90vh]" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border)' }}>
            <h2 className="font-semibold mb-4" style={{ color: 'var(--text-primary)' }}>Edit Property</h2>
            <form onSubmit={handleEdit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {field('Property Name *', 'name', property.name, true)}
              {field('Address Line 1 *', 'addressLine1', property.addressLine1, true)}
              {field('Address Line 2', 'addressLine2', property.addressLine2 ?? '')}
              {field('City *', 'city', property.city, true)}
              {field('State *', 'state', property.state, true)}
              {field('Zip *', 'zip', property.zip, true)}
      ...
      {confirming && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.6)' }}>
          <div className="rounded-xl p-6 w-full max-w-sm shadow-xl text-center" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border)' }}>
            <p className="font-semibold mb-2" style={{ color: 'var(--text-primary)' }}>Remove Property?</p>
            <p className="text-sm mb-4" style={{ color: 'var(--text-muted)' }}>
              <span style={{ color: '#d4a017' }}>{property.name}</span> will be deactivated and hidden from lists — its units and lease history are kept, not deleted.
            </p>
