...
type Account = { id: string; glNumber: string; glName: string; glType: string }
...
export function ChartOfAccountRowActions({ account }: { account: Account }) {
  ...
  async function handleEdit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const res = await fetch(`/api/setup/chart-of-accounts/${account.id}`, {
      method: 'PATCH',
  ...
  async function handleDelete() {
    setLoading(true)
    const res = await fetch(`/api/setup/chart-of-accounts/${account.id}`, { method: 'DELETE' })
    const json = await res.json().catch(() => ({}))
      ...
      {editing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.6)' }}>
          <div className="rounded-xl p-6 w-full max-w-md shadow-xl overflow-y-auto max-h-[90vh]" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border)' }}>
            <h2 className="font-semibold mb-4" style={{ color: 'var(--text-primary)' }}>Edit Account</h2>
            <form onSubmit={handleEdit} className="grid grid-cols-1 gap-4">
              <div>
                ...
                <input name="glNumber" defaultValue={account.glNumber} required className={cls} style={sx} />
              </div>
              <div>
                <label className="block text-xs font-medium mb-1" style={{ color: 'var(--text-muted)' }}>GL Name *</label>
                <input name="glName" defaultValue={account.glName} required className={cls} style={sx} />
              </div>
              <div>
                <label className="block text-xs font-medium mb-1" style={{ color: 'var(--text-muted)' }}>Type *</label>
                <select name="glType" defaultValue={account.glType} required className={cls} style={sx}>
      ...
      {confirming && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.6)' }}>
          <div className="rounded-xl p-6 w-full max-w-sm shadow-xl text-center" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border)' }}>
            <p className="font-semibold mb-2" style={{ color: 'var(--text-primary)' }}>Remove Account?</p>
            <p className="text-sm mb-4" style={{ color: 'var(--text-muted)' }}>
              <span style={{ color: '#d4a017' }}>{account.glNumber} {account.glName}</span> will be deactivated and hidden from pickers — existing transactions referencing it are kept, not deleted.
            </p>
