...
export function OwnerRowActions({ owner }: { owner: Owner }) {
  ...
  async function handleEdit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const res = await fetch(`/api/setup/owners/${owner.id}`, {
      method: 'PATCH',
  ...
  async function handleDelete() {
    setLoading(true)
    const res = await fetch(`/api/setup/owners/${owner.id}`, { method: 'DELETE' })
    const json = await res.json().catch(() => ({}))
      ...
      {editing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.6)' }}>
          <div className="rounded-xl p-6 w-full max-w-md shadow-xl overflow-y-auto max-h-[90vh]" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border)' }}>
            <h2 className="font-semibold mb-4" style={{ color: 'var(--text-primary)' }}>Edit Owner</h2>
            <form onSubmit={handleEdit} className="grid grid-cols-1 gap-4">
              <div>
                ...
                <input name="name" defaultValue={owner.name} required className={cls} style={sx} />
              </div>
              <div>
                <label className="block text-xs font-medium mb-1" style={{ color: 'var(--text-muted)' }}>Type</label>
                <select name="type" defaultValue={owner.type} className={cls} style={sx}>
              ...
              <div>
                ...
                <input name="email" defaultValue={owner.email ?? ''} className={cls} style={sx} />
              </div>
              <div>
                <label className="block text-xs font-medium mb-1" style={{ color: 'var(--text-muted)' }}>Phone</label>
                <input name="phone" defaultValue={owner.phone ?? ''} className={cls} style={sx} />
              </div>
      ...
      {confirming && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.6)' }}>
          <div className="rounded-xl p-6 w-full max-w-sm shadow-xl text-center" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border)' }}>
            <p className="font-semibold mb-2" style={{ color: 'var(--text-primary)' }}>Remove Owner?</p>
            <p className="text-sm mb-4" style={{ color: 'var(--text-muted)' }}>
              <span style={{ color: '#d4a017' }}>{owner.name}</span> will be deactivated and hidden from lists — existing PropertyOwner assignments are kept, not deleted.
            </p>
