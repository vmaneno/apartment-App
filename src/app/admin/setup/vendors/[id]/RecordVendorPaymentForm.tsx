...
export function RecordVendorPaymentForm({ vendorId, balance, bankAccounts }: {
  vendorId: string
  balance: number
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const res = await fetch('/api/ap/vendor-payments', {
      method: 'POST',
  ...
  if (bankAccounts.length === 0) {
    return (
      <p className="text-xs" style={{ color: 'var(--text-muted)' }}>
        No bank account set up yet. <Link href="/admin/setup/bank-accounts" className="underline" style={{ color: 'var(--accent)' }}>Add one in Setup → Bank Accounts</Link> before recording a payment.
      </p>
    )
  }
  ...
  return (
    <div className="rounded-xl p-5 shadow-sm mb-4" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border)' }}>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-2 md:grid-cols-5 gap-3">
        ...
        <div>
          <label className="block text-xs font-medium mb-1" style={{ color: 'var(--text-muted)' }}>Pay From *</label>
