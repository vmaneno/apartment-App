...
export function WorkOrderRowActions({ workOrder, vendors }: { workOrder: WorkOrder; vendors: Vendor[] }) {
  ...
  async function handleEdit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const res = await fetch(`/api/ops/work-orders/${workOrder.id}`, {
      method: 'PATCH',
  ...
  async function handleDelete() {
    setLoading(true)
    const res = await fetch(`/api/ops/work-orders/${workOrder.id}`, { method: 'DELETE' })
    const json = await res.json().catch(() => ({}))
      ...
      {editing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.6)' }}>
          <div className="rounded-xl p-6 w-full max-w-md shadow-xl overflow-y-auto max-h-[90vh]" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border)' }}>
            <h2 className="font-semibold mb-4" style={{ color: 'var(--text-primary)' }}>Edit Work Order</h2>
            <form onSubmit={handleEdit} className="grid grid-cols-1 gap-4">
              <div>
                ...
                <input name="description" defaultValue={workOrder.description} required className={cls} style={sx} />
              </div>
              <div>
                <label className="block text-xs font-medium mb-1" style={{ color: 'var(--text-muted)' }}>Priority</label>
                <select name="priority" defaultValue={workOrder.priority} className={cls} style={sx}>
              ...
              <div>
                ...
                <select name="status" defaultValue={workOrder.status} className={cls} style={sx}>
              ...
              <div>
                ...
                <select name="assignedVendorId" defaultValue={workOrder.assignedVendorId ?? ''} className={cls} style={sx}>
      ...
      {confirming && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.6)' }}>
          <div className="rounded-xl p-6 w-full max-w-sm shadow-xl text-center" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border)' }}>
            <p className="font-semibold mb-2" style={{ color: 'var(--text-primary)' }}>Delete Work Order?</p>
            <p className="text-sm mb-4" style={{ color: 'var(--text-muted)' }}>
              <span style={{ color: '#d4a017' }}>{workOrder.description}</span> will be permanently removed.
            </p>
